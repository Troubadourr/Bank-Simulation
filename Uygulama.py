...
def hesap_olustur():
    ...
    def kaydet_hesap():
        ...
        # ✅ En az 6 haneli ID kontrolü
        if not kullanici.isdigit() or len(kullanici) < 6:
            messagebox.showerror("Hata", "Kullanıcı ID en az 6 haneli ve sadece rakamlardan oluşmalıdır.")
            return
    ...
    tk.Button(pencere, text="Kaydet", font=("Segoe UI", 10, "bold"), width=15,
              bg="#1565C0", fg="white", command=kaydet_hesap).pack(pady=5)

    tk.Button(pencere, text="Kapat", font=("Segoe UI", 10, "bold"), width=15,
              bg="#D32F2F", fg="white", command=pencere.destroy).pack(pady=5)

    tk.Button(pencere, text="Geri", font=("Segoe UI", 10, "bold"), width=15,
              bg="#90A4AE", fg="white", command=pencere.destroy).pack()
